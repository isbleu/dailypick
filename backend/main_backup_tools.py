...
def main():
    ...
    parser.add_argument("--api", type=str, choices=["glm", "openrouter"], default="glm",
                        help="选择大模型 API 通道: glm=智谱官方(默认), openrouter=OpenRouter代理(联网搜索更强)。")
    ...
    # 3. 盘前数据拉取与解析
    # 3.1 龙虎榜数据 (拉取前一个有效交易日的数据，如周一早晨选股参考上周五的龙虎榜)
    ...
    # 校验基础输入是否存在
    if not pdf_text and lhb_df.empty:
        print("[错误] 核心星球简报 PDF 与龙虎榜机构数据均缺失，系统无法判定选股逻辑，终止运行。")
        return
    ...
    # 5.3 同时复制一份到工作区的 examples/ 目录下以满足样本归档
