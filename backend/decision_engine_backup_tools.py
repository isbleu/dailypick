...
class DecisionEngine:
    ...
    def _call_llm(cls, system_prompt: str, user_prompt: str) -> str:
        ...
        if is_zhipu_official:
            ...
            # OpenRouter：reasoning map + reasoning_effort
            data["reasoning"] = {
                "effort": "medium",
        ...
        # 尝试开启联网搜索工具
        if "glm" in Config.LLM_MODEL.lower():
            if is_openrouter:
                data["tools"] = [
                    {
        ...
        for attempt in range(1, max_retries + 1):
            ...
            try:
                ...
                if start_idx != -1:
                    # 倒序查找所有 '}'，逐个尝试 JSON 解析，防止尾部有 ```" } ``` 等混淆符号
                    ...
                    for end_idx in reversed(end_indices):
                        if end_idx > start_idx:
                            ...
                            try:
                                ...
                                content = candidate  # 成功解析，这就是最完整、正确的 JSON 边界
                                break
                            except Exception:
                                pass
                return content
